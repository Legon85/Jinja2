...
# НАСЛЕДОВАНИЕ шаблонов позволяет уменьшать объём дублируемого кода в шаблонах.

# Для простоты восприятия создадим новый файл с шаблоном ex_main.htm в подкаталоге templates в котором используется
# новый тип блоков - именованные блоки:{% block title %}{% endblock %}, {% block content %} {% endblock %}.
# <!DOCTYPE html>
# <html>
# <head>
#     <meta charset="utf-8">
...
# {% extends 'layout/default.tpl' %} и тогда всё будет работать в том же виде.
print(output)
print("-" * 60)
...
# В базовых шаблонах так же можно делать вложенные блоки, вызываемые так же через super() соответственно.
