...
# далее в теге <imput> все элементы value, type, size принимают соответствующие значения из блока выше обозначенного
# блока, при чём к value дополнительно применяется флаг экранирования e.
# Далее формируем шаблон, в котором можем использовать макрос множество раз.
# В теге абзаца <p> формируем тег <imput> из макроса. И указываем что поле 'name' из макроса будет принимать значение
